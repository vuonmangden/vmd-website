...
| FND-002 | Local development | Review | FND-001 | `chore/fnd-002-local-development` / `01ce62e` | Không | Node 24.14.0; pnpm 11.9.0 frozen install đạt; lint/typecheck/test/build 12/12 đạt, cache bypass; Compose config đạt; 6/6 service healthy; infra:check đạt trước/sau restart | Loopback-only ports; pinned images; không latest/privileged/socket/secret; env boundary và ignore checks đạt | PostgreSQL query, Redis ping, Mailpit SMTP/UI, MinIO bucket, Mock SePay/Zalo success/error đạt; down giữ 4 named volumes |
