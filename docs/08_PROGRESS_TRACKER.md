...
| Task đang thực hiện | Không có task nào đang code dở; `BBQ-003` vừa triển khai xong, chờ review |
| Task hoàn thành | 58 Done/Review (+3: `BBQ-001`, `PAY-004`, `ADM-001` merged 2026-08-19; `BBQ-003` Review 2026-08-20) — mở khóa tiếp `BBQ-004` (BBQ Booking) khi `BBQ-002`/`BKG-001` đã Done; `PAY-005` (Admin Payment) gần đủ điều kiện |
...
| PAY-005 | Admin Payment | Backlog | PAY-004, IAM-003, IAM-004 |  |  | `PAY-004` ở Review, sắp đủ điều kiện — dashboard/list view tổng hợp payment intent + reconciliation case (`ReconciliationController` đã có API nền `admin/payments/reconciliation-cases`) |
