...
export function CheckoutForm() {
  ...
  const [form, setForm] = useState<CheckoutPayload>({ ...initial, fullName: '', phone: '', email: '', extraMattressQuantity: 0, bookingPolicyAccepted: false, privacyPolicyAccepted: false, specialRequest: '', expectedArrivalTime: '' });
  ...
  return <form className="checkout-form" onSubmit={submit} aria-describedby={error ? 'checkout-error' : undefined}>
    ...
    {step >= 2 && <fieldset><legend>Thông tin khách</legend><div className="checkout-grid"><label>Họ và tên<input required maxLength={150} value={form.fullName} onChange={(e) => update('fullName', e.target.value)} /></label><label>Số điện thoại Việt Nam<input required inputMode="tel" pattern="(?:\\+84|84|0)(?:3|5|7|8|9)[0-9]{8}" value={form.phone} onChange={(e) => update('phone', e.target.value)} /></label><label>Email <small>(khuyến nghị)</small><input type="email" value={form.email} onChange={(e) => update('email', e.target.value)} /></label><label>Giờ đến dự kiến <small>(tuỳ chọn)</small><input maxLength={50} value={form.expectedArrivalTime} onChange={(e) => update('expectedArrivalTime', e.target.value)} /></label><label>Đệm thêm<select value={form.extraMattressQuantity} onChange={(e) => update('extraMattressQuantity', Number(e.target.value))}><option value={0}>Không kê thêm</option><option value={1}>01 đệm · 200.000 VND/đêm</option></select></label></div><label>Yêu cầu đặc biệt <small>(tuỳ chọn)</small><textarea maxLength={1000} value={form.specialRequest} onChange={(e) => update('specialRequest', e.target.value)} /></label><button type="button" className="button button-secondary" onClick={() => setStep(3)}>Xem lại &amp; thanh toán</button></fieldset>}
    {step >= 3 && <fieldset><legend>Xác nhận đặt phòng</legend><p className="room-message">Phòng được giữ 30 phút kể từ khi tạo booking. Hệ thống sẽ tính lại giá và mức cọc 50% hoặc 100% theo ngày lưu trú. Không nhập CCCD/hộ chiếu trên trang này.</p><label className="checkout-consent"><input required type="checkbox" checked={form.bookingPolicyAccepted} onChange={(e) => update('bookingPolicyAccepted', e.target.checked)} /> Tôi đồng ý chính sách đặt phòng, đổi/hủy và hoàn tiền.</label><label className="checkout-consent"><input required type="checkbox" checked={form.privacyPolicyAccepted} onChange={(e) => update('privacyPolicyAccepted', e.target.checked)} /> Tôi đồng ý chính sách xử lý dữ liệu cá nhân.</label><button className="button button-primary" disabled={busy} type="submit">{busy ? 'Đang tạo đặt phòng…' : 'Tạo đặt phòng & thanh toán'}</button></fieldset>}
